...
class block_member extends discuz_block {
	...
	function fields() {
		...
		$fields = array(
                ...
                'customstatus' => array('name' => '自定义头衔', 'formtype' => 'text', 'datatype' => 'string'),
				);
	...
	function getdata($style, $parameter) {
		...
		if($resultuids) {
            ...
            $userForumInfo = C::t('common_member_field_forum')->fetch_all($resultuids);
            for($i=0,$L=count($list); $i<$L; $i++) {
                $uid = $list[$i]['id'];
                if($userForumInfo[$uid]) {
                    $list[$i]['fields']['customstatus'] = $userForumInfo[$uid]['customstatus'];
                }
            }
			for($i=0,$L=count($list); $i<$L; $i++) {
				$uid = $list[$i]['id'];
