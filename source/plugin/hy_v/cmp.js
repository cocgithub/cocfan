...
function hy_v_add_plugin(v, plugin){if(!v.plugins){v.plugins = "";} plugin = plugin.replace(/(^\s*|\s*$)/, ""); var parr = v.plugins.split(","); var is_exists = 0; for(var i in parr){if(parr[i].replace(/(^\s*|\s*$)/, "") == plugin){is_exists = 1;}} if(is_exists == 0){parr.push(plugin);} v.plugins = parr.join(",");}
function hy_v_add_model(v, model){if(!v.models){v.models = "";} model = model.replace(/(^\s*|\s*$)/, ""); var parr = v.models.split(","); var is_exists = 0; for(var i in parr){if(parr[i].replace(/(^\s*|\s*$)/, "") == model){is_exists = 1;}} if(is_exists == 0){parr.push(model);} v.models = parr.join(",");}
function cloneobj(obj){var tmp = {};for(var i in obj){tmp[i] = obj[i];}return tmp;}