...
class threadplugin_auction {
	...
	function editpost_submit_end($fid, $tid) {
		...
		if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('plugin_auction')." WHERE tid='$tid'")) {
			...
			if($auc['starttimeto']) {
				$auction = DB::fetch_first("SELECT * FROM ".DB::table('plugin_auction')." WHERE tid='$tid'");
				if($auc['starttimeto'] < $auction['starttimeto'] || $auc['starttimeto'] < $_G['timestamp']) {
            ...
            if($auc['number'] > 0){
                DB::query("UPDATE ".DB::table('plugin_auction')." SET number='{$auc[number]}' WHERE tid='$tid'");
            }
		}
	}
	...
	function check_gpc() {
		...
		if($auc['starttimeto'] - $auc['starttimefrom'] <= 120) {
