...
if($action == 'index') {
    ...
    $userJoinGroups = C::t('forum_groupuser')->fetch_all_group_for_user($_G['uid']);
    if($groupuser['uid']) {
        showmessage('group_has_joined', "forum.php?mod=group&fid=$_G[fid]");
    }else if(!empty($userJoinGroups)){
        showmessage('group_join_just_once', "forum.php?mod=group&fid=$_G[fid]");
    } else {
		$modmember = 4;
    ...
    if(!empty($userJoinGroups)){
        showmessage('group_join_just_once', "group.php");
    }
