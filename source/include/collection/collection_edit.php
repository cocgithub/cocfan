...
$oplist = array('add', 'edit', 'remove', 'addthread', 'delthread', 'acceptinvite', 'removeworker', 'invite','addgroup');
...
if(empty($op) || $op == 'add') {
    ...
    if(!submitcheck('addgroup')) {
        $myGroups = C::t('forum_groupuser')->fetch_all_group_for_user($_G['uid'],0,1);
        if(empty($myGroups)){
            showmessage('对不起，您不是部落团长，请让部落团长来分享该帖子吧');
        }
        $fids = array();
        foreach($myGroups as $group){
            $fids[] = $group['fid'];
        }
        $groupNames = C::t('forum_forum')->fetch_all_name_by_fid($fids);
        foreach($myGroups as &$group){
            $group['name'] = $groupNames[$group['fid']]['name'];
        }
        include template('forum/collection_groupselect');
    } else {
        $fid = $_GET['fid'];
        if(empty($fid)){
            showmessage('请选择部落', '', array(), array('showdialog' => 1));
        }
        $groupInfo = C::t('forum_forum')->fetch($fid);
        if(empty($groupInfo) || $groupInfo['status'] != 3){
            showmessage('您选择的部落不存在', '', array(), array('showdialog' => 1));
        }
        $userGroupInfo = C::t('forum_groupuser')->fetch_userinfo($_G['uid'],$fid);
        if(empty($userGroupInfo) || $userGroupInfo['level'] > 2){
            showmessage('您没有分享帖子到部落的权限', '', array(), array('showdialog' => 1));
        }
        $nThreadInfo = C::t('forum_thread')->fetch_by_fid_closetid($fid,$_G['thread']['tid']);
        if(!empty($nThreadInfo)){
            showmessage('该帖子已经分享过到您的部落', '', array(), array('showdialog' => 1));
        }
        $thread = $_G['thread'];
        $insertdata = array(
            'fid' => $fid,
        ...
        showmessage('添加文章成功', dreferer(), array(), array('alert'=> 'right', 'closetime' => true, 'locationtime' => true, 'showdialog' => 1));
    }
