...
<div style="font-size: 14px;line-height: 24px;">
			...
			<div class="z" style="width:80px"><img src="$task[icon]" alt="Icon" width="64" height="64" /></div>
		...
		<div class="clear"></div>
			...
			<table cellpadding="0" cellspacing="0" class="dsm_tfm" style="margin-top:10px;">
					<tr>
				...
				<!--{if $allowapply == '-1'}-->
					<div style="height: 31px;"><div class="dsm_tapbg mtm mbm">
					...
					<div class="clear"></div>
					<p class="mbw">
						<a href="home.php?mod=task&do=draw&id=$task[taskid]" class="dsm_aniu {if $task[csc] >=100}dsm_tapply{else}&nbsp;{/if}">{if $task[csc] >=100}交任务{else}任务未完成{/if}</a>
						<!--{if $task[csc] < 100}--><a href="home.php?mod=task&do=delete&id=$task[taskid]" class="dsm_aniu dsm_tapply" title="{lang task_quit}" style="margin-top: 10px;">放弃任务</a><!--{/if}-->
					</p>
				...
				<!--{elseif $allowapply == '-3'}-->
					...
					<a href="javascript:;" onclick="doane(event);showDialog('{lang task_applies_full}')" class="dsm_aniu" title="{lang task_applies_full}">接取人数已达上限</a>
				<!--{elseif $allowapply == '-4'}-->
				...
				<!--{elseif $allowapply == '-6'}-->
					...
					<a href="javascript:;" onclick="doane(event);showDialog('{$task[t]}{lang task_applyagain}')" class="dsm_aniu" title="{$task[t]}{lang task_applyagain}，{lang task_applies_full}">{$task[t]}{lang task_applyagain}</a>
				<!--{elseif $allowapply == '-7'}-->
				...
				<!--{if $allowapply > '0'}-->
					<a href="home.php?mod=task&do=apply&id=$task[taskid]" class="dsm_aniu dsm_tapply" title="{lang task_newbie_apply}">接任务</a>
				<!--{/if}-->
